...
from pcluster.schemas.validators import efs_existence_of_mode_throughput_validator
...
def _kms_key_id():
    # This is an experimental function to simplified the code a little, because kms_key_id is used in multiple places.
    return fields.Str(data_key="KmsKeyId")
...
class EphemeralVolumeSchema(BaseSchema):
    ...
    encrypted = fields.Bool()
    mount_dir = fields.Str(validate=_get_validator("file_path"))
...
class EfsSchema(BaseSchema):
    ...
    def validate_existence_of_mode_throughput(self, data, **kwargs):
        ...
        errors = efs_existence_of_mode_throughput_validator(
            data.get("throughput_mode"), data.get("provisioned_throughput")
        )
        if errors:
            raise ValidationError(message=errors, field_name="ThroughputMode")
...
class FsxSchema(BaseSchema):
    ...
    @validates("imported_file_chunk_size")
    def fsx_imported_file_chunk_size_validator(self, value):
        """Validate ImportedFileChunkSize."""
        if not 1 <= value <= 512000:
            raise ValidationError("has a minimum size of 1 MiB, and max size of 512,000 MiB")
...
class SharedStorageSchema(BaseSchema):
    ...
    mount_dir = fields.Str(required=True, validate=_get_validator("file_path"))
    ebs_settings = fields.Nested(EbsSchema, data_key="EBS")
    efs_settings = fields.Nested(EfsSchema, data_key="EFS")
    fsx_settings = fields.Nested(FsxSchema, data_key="FSX")
    ...
    def validate_not_none_dir(self, value):
        ...
        if re.match("^/?NONE$", value):
            raise ValidationError("{0} cannot be used as a mount directory".format(value))

    @validates_schema
    def validate_existence_of_settings(self, data, **kwargs):
        """Validate that there is a setting."""
        pass

    @validates_schema
    def validate_only_one_settings(self, data, **kwargs):
        """Validate that there is no more than one settings."""
        pass
...
class BaseNetworkingSchema(BaseSchema):
    ...
    additional_security_groups = fields.List(fields.Str(validate=_get_validator("security_group_id")))
    assign_public_ip = fields.Bool()
    security_groups = fields.List(fields.Str(validate=_get_validator("security_group_id")))
...
class HeadNodeNetworkingSchema(BaseNetworkingSchema):
    ...
    subnet_id = fields.Str(required=True, validate=_get_validator("subnet_id"))
...
class QueueNetworkingSchema(BaseNetworkingSchema):
    ...
    subnet_ids = fields.List(fields.Str(validate=_get_validator("subnet_id")), required=True)
...
class SshSchema(BaseSchema):
    ...
    allowed_ips = fields.Str(validate=_get_validator("cidr"))
...
class IamSchema(BaseSchema):
    ...
    roles = fields.Nested(RolesSchema, data_key="Roles")
...
class ClusterSchema(BaseSchema):
    ...
    # custom_action_list = fields.Nested(CustomActionSchema, data_key="CustomActions", many=True)
