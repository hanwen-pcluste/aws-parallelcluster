...
def _check_in_out_access(security_groups_ids, port, cidr_optional):
    """
    ...
    :param cidr_optional: if it is True, don't enforce check on CIDR.
    ...
    for sec_group in AWSApi.instance().ec2.describe_security_groups(security_groups_ids).get("SecurityGroups"):
        ...
        for rule in sec_group.get("IpPermissions"):
            if _check_sg_rules_for_port(rule, port):
                if cidr_optional or rule.get("IpRanges"):
                    in_access = True
                    break
...
class FsxNetworkingValidator(Validator):
    ...
    def _validate(self, file_system_id, head_node_subnet_id, is_all_security_group_customized):
        try:
            ...
            if not network_interface_ids:
                ...
                for network_interface in network_interfaces:
                    ...
                    if _check_in_out_access(sg_ids, port=988, cidr_optional=is_all_security_group_customized):
                        fs_access = True
                        break
                if not fs_access:
                    self._add_failure(
...
class EfsIdValidator(Validator):  # TODO add tests
    ...
    def _validate(self, efs_id, head_node_avail_zone: str, is_all_security_group_customized):
        ...
        if head_node_target_id:
            ...
            if not _check_in_out_access(sg_ids, port=2049, cidr_optional=is_all_security_group_customized):
                self._add_failure(
