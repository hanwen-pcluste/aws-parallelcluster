...
class ExistingFsxNetworkingValidator(Validator):
    ...
    def _validate(self, file_system_ids, head_node_subnet_id, are_all_security_groups_customized):
        try:
            ...
            # Cache describe network interfaces boto3 call
            all_network_interfaces = []
            for file_system in file_systems:
                all_network_interfaces.extend(file_system.file_system_data.get("NetworkInterfaceIds"))
            ...
            for file_system in file_systems:
                ...
                file_system_data = file_system.file_system_data
                file_system_id = file_system_data.get("FileSystemId")
                if file_system_data.get("VpcId") != vpc_id:
                    self._add_failure(
                        "Currently only support using FSx file system that is in the same VPC as the cluster. "
                        "The file system provided is in {0}.".format(file_system_data.get("VpcId")),
                        FailureLevel.ERROR,
                    )

                # If there is an existing mt in the az, need to check the inbound and outbound rules of the security groups
                network_interface_ids = file_system_data.get("NetworkInterfaceIds")
