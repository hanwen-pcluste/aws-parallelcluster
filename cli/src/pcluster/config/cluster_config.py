...
class BaseClusterConfig(Resource):
                ...
                if isinstance(storage, SharedFsx):
                    ...
                    if storage.file_system_id:
                        self._register_validator(
                            ...
                            is_all_security_group_customized=self.is_all_security_group_customized,
                        )
                    self._register_validator(
    ...
    @property
    def is_all_security_group_customized(self):
        """Return True if all of head node and queues have (additional) security groups specified."""
        head_node_networking = self.head_node.networking
        if not (head_node_networking.security_groups or head_node_networking.security_groups):
            return False
        for queue in self.scheduling.queues:
