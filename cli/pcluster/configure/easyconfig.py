...
from pcluster.config.validators import HEADNODE_UNSUPPORTED_INSTANCE_TYPES
...
def _is_instance_type_supported_for_head_node(instance_type):
    if instance_type in HEADNODE_UNSUPPORTED_INSTANCE_TYPES:
        print("The instance type '{0}' is not supported as head node.".format(instance_type))
        return False
    return True
...
class ClusterConfigureHelper:
    ...
    def prompt_instance_types(self):
        ...
        self.master_instance_type = prompt(
            ...
            lambda x: x in get_supported_instance_types() and _is_instance_type_supported_for_head_node(x),
