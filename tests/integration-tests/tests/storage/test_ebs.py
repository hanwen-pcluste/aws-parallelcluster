...
def _get_root_volume_id(instance_id, os, region):
    block_device_mappings = boto3.client("ec2", region_name=region).describe_instances(InstanceIds=[instance_id])[
        "Reservations"
    ][0]["Instances"][0]["BlockDeviceMappings"]
    for mapping in block_device_mappings:
        if mapping["DeviceName"] == OS_TO_ROOT_VOLUME_DEVICE[os]:
...
def _test_root_volume_encryption(cluster, os, region, scheduler, encrypted):
    ...
    if scheduler == "slurm":
        ...
        for instance in instance_ids:
            root_volume_id = _get_root_volume_id(instance, os, region)
        ...
        root_volume_id = _get_root_volume_id(cluster.cfn_resources["HeadNode"], os, region)
        _test_ebs_encrypted_with_kms(root_volume_id, region, encrypted=encrypted)
