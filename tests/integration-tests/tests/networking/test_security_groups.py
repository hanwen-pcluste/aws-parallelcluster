...
def _check_connections_between_head_node_and_compute_nodes(cluster):
    ...
    for compute_node in compute_nodes:
        ...
        head_node_remote_command_executor.run_remote_command(f"ping {compute_node_ip} -c 5")
        ...
        compute_remote_command_executor.run_remote_command(f"ping {head_node_private_ip} -c 5")
        for other_compute_node in compute_nodes:
            other_compute_node_ip = other_compute_node.get("privateIpAddress")
            compute_remote_command_executor.run_remote_command(f"ping {other_compute_node_ip} -c 5")
